...
var Scoreboard = new function () {
    ...
    self.init_filters = function () {
        self.filter_recent_el = $('#ScoreboardFilter_recent');
        self.filter_tasks_el = $('#ScoreboardFilter_tasks');
        self.filter_contests_el = $('#ScoreboardFilter_contests');

        self.filter_tasks_el.on("change", function () {
            self.filters.show_tasks = $(this).is(":checked");
            self.apply_filters();
        });

        self.filter_tasks_el.prop("checked", self.filters.show_tasks);
        self.apply_filters();
    };

    self.render_contest_buttons = function () {
        if (self.filter_contests_el.length === 0) {
            return;
        }
        ...
        for (var i = 0; i < contests.length; i += 1) {
            var c_id = String(contests[i]["key"]);
            var label = "第" + (i + 1) + "場";
            var active = (self.filters.recent === "contest:" + c_id) ? " active" : "";
            html += "<button type=\"button\" class=\"ScoreboardFilter_btn" + active + "\" data-contest=\"" + c_id + "\">" + label + "</button>";
        }
        self.filter_contests_el.html(html);

        self.filter_contests_el.off("click", ".ScoreboardFilter_btn");
        self.filter_contests_el.on("click", ".ScoreboardFilter_btn", function () {
            var c_id = String($(this).data("contest"));
            self.filters.recent = "contest:" + c_id;
            self.apply_filters();
        });
    };
    ...
    self.apply_filters = function () {
        ...
        if (self.filters.recent === "last1") {
            if (contests.length > 0) {
                visible[String(contests[contests.length - 1]["key"])] = true;
            }
        } else if (self.filters.recent.indexOf("contest:") === 0) {
            var cid = self.filters.recent.slice("contest:".length);
            if (cid !== "") {
                visible[String(cid)] = true;
            }
        } else if (self.filters.recent === "last3") {
            var last_n = Math.min(3, contests.length);
            var last_start = Math.max(0, contests.length - last_n);
            for (var l = last_start; l < contests.length; l += 1) {
                visible[String(contests[l]["key"])] = true;
            }
        } else if (self.filters.recent === "all") {
            for (var a = 0; a < contests.length; a += 1) {
                visible[String(contests[a]["key"])] = true;
            }
        } else {
            var n = parseInt(self.filters.recent, 10);
            var start = Math.max(0, contests.length - n);
            for (var j = start; j < contests.length; j += 1) {
                visible[String(contests[j]["key"])] = true;
            }
        }

        self.visible_contests = visible;
        self.apply_visibility();
        self.render_contest_buttons();
    };

    self.set_hidden = function ($el, hidden) {
        if (hidden) {
            $el.addClass("sb-hidden");
        } else {
            $el.removeClass("sb-hidden");
        }
    };

    self.apply_visibility = function () {
        var visible = self.visible_contests || {};
        var show_tasks = self.filters.show_tasks;

        $("col.score.contest, th.score.contest, td.score.contest").each(function () {
            var $this = $(this);
            var c_id = String($this.data("contest"));
            self.set_hidden($this, !visible[c_id]);
        });

        $("col.score.task, th.score.task, td.score.task").each(function () {
            var $this = $(this);
            var t_id = $this.data("task");
            var contest_id = String(DataStore.tasks[t_id]["contest"]);
            var hide = !show_tasks || !visible[contest_id];
            self.set_hidden($this, hide);
        });
    };
